...
function ProjectItem({ project, accent, isSelected, onClick }) {
  return (
    <button
      onClick={onClick}
      className={[
        "w-full text-left px-3 py-2.5 rounded-xl border transition-all duration-150 flex items-center gap-3 cursor-pointer",
        "dark:border-[#2e3545]",
        isSelected
          ? `bg-white dark:bg-[#1a1f2b] shadow-sm ring-2 ${accent.ring} ${accent.border} dark:border-transparent`
          : "bg-white dark:bg-[#1e2433] border-gray-200 hover:bg-slate-50 dark:hover:bg-[#252d3d] hover:border-gray-300 dark:hover:border-[#31f64b]/30",
      ].join(" ")}
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[11px] text-slate-400 dark:text-slate-500 truncate gap-3 flex">
          {project.client ?? "No client"} ·{" "}
          <span className={`font-semibold rounded ${STATUS_STYLE[project.status ?? "not started"]}`}>
  ...
  return (
    <div
      ...
      className={[
        ...
        hov && cell.isCurrentMonth && !isSelected ? "bg-slate-50 dark:bg-[#252d3d]" : "",
...
function DayDetail({ selectedDate, logs, loading, project, accent }) {
  ...
  if (!selectedDate) {
    return (
      <div className="flex flex-col items-center justify-center h-full py-16 text-slate-400 dark:text-slate-600 text-center px-6">
          ...
          return (
            <div
              ...
              className={[
                ...
                "hover:bg-gray-50 dark:hover:bg-[#252d3d] transition-colors duration-150",
                "border-l-4",
                PRIORITY_BORDER[log.task?.priority] ?? "border-l-slate-300",
              ].join(" ")}
            >
...
export default function AdminDashboard() {
  ...
  // Shared dark-mode control styles
  const darkControl = [
  ...
  return (
    <div className="h-full w-full flex flex-col rounded-2xl bg-slate-100 dark:bg-[#181d28] font-sans">
      ...
      <header className="bg-white dark:bg-[#222732] border-b border-slate-200 dark:border-[#2a3040] px-4 lg:px-6 py-3.5
        ...
        <div className="flex items-center gap-3">
          {/* Accent bar — green in dark mode */}
          <div className="w-1 h-7 rounded-full bg-blue-600 dark:bg-[#31f64b]" />
          <h1 className="text-base md:text-lg font-extrabold tracking-tight dark:text-slate-100">
            Project Calendar
          </h1>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <select
            ...
            className={`border-2 border-slate-200 rounded-lg px-3 py-2 text-sm font-semibold
          ...
          <button
            onClick={() => navMonth(-1)}
            className={`w-9 h-9 border-2 border-slate-200 flex items-center justify-center rounded-lg
          ...
          <button
            ...
            className={`px-4 h-9 border-2 border-slate-200 rounded-lg bg-white text-sm font-semibold
      ...
      <div className="flex flex-col lg:flex-row" style={{ height: "calc(100vh - 57px)" }}>
        ...
        <aside className="w-full lg:w-64 xl:w-72 bg-white dark:bg-[#222732] border-b lg:border-b-0 lg:border-r
          ...
          <div className="flex-1 overflow-y-auto px-3 py-2.5 flex flex-col gap-2 dark:bg-[#222732]">
            ...
            {!projLoading && projects.length === 0 && (
              <p className="text-xs text-slate-400 dark:text-slate-600 text-center py-10">No projects found.</p>
            )}
          </div>
