import { XCircle } from "lucide-react";
import { Fragment } from "react";

function AddTheManagerFromDepartment({ open, setOpen, department, managers, onAddManager }) {
  const labelCls = "block text-sm font-medium text-gray-700 dark:text-slate-300";

  const filteredManagers = managers.filter((manager) => manager.department?.id === department.id);

  return (
    <Fragment>
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white gap-2 dark:bg-[#222732] rounded-xl shadow-xl dark:shadow-[0_4px_40px_rgba(0,0,0,0.6)] w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-[#2a3040]">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-slate-100">Add Manager From Department</h2>
              <button className="p-2 hover:bg-gray-100 dark:hover:bg-[#252d3d] rounded-lg transition-colors" onClick={() => setOpen(false)}>
                <XCircle size={24} className="text-gray-500 dark:text-slate-400 cursor-pointer" />
              </button>
            </div>

            <div className="p-6">
              <div className="space-y-4">
                <div className="space-y-2">
                  <label className={labelCls}>Department</label>
                  <input
                    type="text"
                    value={department?.name || "--"}
                    disabled
                    className="w-full px-3 py-2 rounded-md text-sm border border-gray-300 dark:border-[#2a3040] bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-slate-200 cursor-not-allowed"
                  />
                </div>

                <div className="space-y-2">
                  <label className={labelCls}>Available Managers</label>

                  {filteredManagers && filteredManagers.length > 0 ? (
                    <div className="flex flex-col w-full max-h-60 border-2 border-gray-200 dark:border-[#2a3040] rounded overflow-y-auto">
                      {filteredManagers.map((manager) => (
                        <div key={manager.id} className="py-2 px-3 border-b border-gray-200 dark:border-[#1f2937] flex items-center justify-between gap-3">
                          <div>
                            <p className="font-medium text-gray-800 dark:text-slate-200">{manager.fullname || manager.name || "Unnamed"}</p>
                            <p className="text-xs text-gray-500 dark:text-gray-400">Role: {manager.role || "manager"}</p>
                            <p className="text-xs text-gray-500 dark:text-gray-400">Dept: {manager.department?.name || "N/A"}</p>
                          </div>
                          <button
                            onClick={() => onAddManager(manager.id)}
                            className="px-2 py-1 text-xs font-semibold text-white bg-blue-600 rounded hover:bg-blue-700"
                          >
                            Add
                          </button>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <div className="p-4 text-sm text-gray-500 dark:text-gray-300">
                      All managers are currently assigned to this project management.
                    </div>
                  )}
                </div>
              </div>
            </div>
          </div>
        </div>
      )}
    </Fragment>
  );
}

export default AddTheManagerFromDepartment;